import XCTest
@testable import CaffeineCutoff

@MainActor
final class CaffeineCutoffTests: XCTestCase {
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(DrinkEntry(note: "test"))
    ...
    func testCanAddBlockedAtLimitWhenNotPro() {
        let store = Store()
        store.entries = (0..<Store.freeTierLimit).map { _ in DrinkEntry() }
        XCTAssertFalse(store.canAdd(isPro: false))
    }

    func testCanAddAlwaysAllowedWhenPro() {
        let store = Store()
        store.entries = (0..<(Store.freeTierLimit + 5)).map { _ in DrinkEntry() }
        XCTAssertTrue(store.canAdd(isPro: true))
    }

    func testDeleteEntryRemovesIt() {
        let store = Store()
        let entry = DrinkEntry(note: "to delete")
        store.add(entry)
        store.delete(entry)
    ...
    func testUpdateEntryPersistsChange() {
        let store = Store()
        var entry = DrinkEntry(note: "original")
        store.add(entry)
        entry.note = "updated"
        store.update(entry)
