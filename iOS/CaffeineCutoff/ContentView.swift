    ...
    private func entryRow(_ entry: DrinkEntry) -> some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                Text(entry.tag.isEmpty ? "Drink" : entry.tag)
...
struct EntryEditorView: View {
    ...
    let onSave: (DrinkEntry) -> Void

    init(entry: DrinkEntry?, onSave: @escaping (DrinkEntry) -> Void) {
        self.existing = entry
        self.onSave = onSave
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(existing == nil ? "Add Drink" : "Edit Drink")
            .toolbar {
