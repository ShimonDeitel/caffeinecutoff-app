...
final class Store: ObservableObject {
    ...
    private let fileName = "caffeinecutoff_entries.json"
    ...
    private func seed() {
        let now = Date()
        entries = [
            DrinkEntry(date: now.addingTimeInterval(-86400*2), note: "First entry", minutes: 10, tag: "General", rating: 2),
            DrinkEntry(date: now.addingTimeInterval(-86400), note: "Second entry", minutes: 15, tag: "General", rating: 3)
        ]
        save()
    }
    ...
    private func load() {
        ...
        if let decoded = try? JSONDecoder().decode([DrinkEntry].self, from: data) {
            entries = decoded
        }
    }
